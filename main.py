  ...
  if response.lower() == 'y':
    if index == 0:
      ...
      for title_index in range(len(test_titles)):
        # Call 'average_for_test' here. Pass in title_index and scores as arguments.
      ...
      for astronaut_index in range(len(astronauts)):
        # Call 'average_for_student' here. Pass in astronaut_index and scores as arguments.
        avg = averages.average_for_student(astronaut_index, scores)
        print("{0}'s test average = {1}%".format(astronauts[astronaut_index], avg))
    else:
      # Call 'random_from_list' to pick a spacewalker from the astronauts list.
      walker = random_select.random_from_list(astronauts)

      print("\n" + walker + " is the next spacewalker.")
  else:
